...
for n = 1:matrix_length
    for o = 1:matrix_length
        ...
        o1.ortho_mat(n,o) = braille_PWLD(char(o1.summary{n,1}),char(o1.summary{o,1}));
        o1.phon_mat(n,o) = levPW(char(o1.summary{n,4}),char(o1.summary{o,4}));
...
% Get triangle
o1.phon_tri = o1.phon_mat(triu(true(size(o1.phon_mat)),1));
o1.ortho_tri = o1.ortho_mat(triu(true(size(o1.ortho_mat)),1));
% Correlate phonologies
o1.r_fon = corrcoef(o1.p_tri,o1.phon_tri);
o1.r_lev = corrcoef(o1.ortho_tri, o1.phon_tri);
...
o1.fr_htmp = heatmap(lab_fr,lab_fr,o1.m_mat,'CellLabelColor','none');   % movies frequency
o1.bk_htmp = heatmap(lab_fr,lab_fr,o1.b_mat,'CellLabelColor','none');   % books frequency
o1.ph_htmp = heatmap(lab_fr,lab_fr,double(o1.p_mat),'CellLabelColor','none');  % phonology
o1.se_htmp = heatmap(lab_fr,lab_fr,double(o1.s_mat),'CellLabelColor','none');  % semantics

f = figure; % to have them in a new figure
% Later addition: pwld
% orthography according to braille_pwld
%o1.ortho_htmp = heatmap(lab_fr,lab_fr,o1.ortho_mat,'CellLabelColor','none','Colormap',parula,'Units','pixels','Position',[60 60 440 440]);
% phonological similarity with index
o1.phon_htmp = heatmap(lab_fr,lab_fr,o1.phon_mat,'Colormap',parula,'Units','pixels','Position',[60 60 440 440]);
...
for n = 1:matrix_length
    for o = 1:matrix_length
        o2R.ortho.lev_br(n,o) = braille_PWLD(char(o2R.words{n,1}),char(o2R.words{o,1})); % ortho
        o2R.phon.lev_fon(n,o) = levPW(char(o2R.words{n,2}),char(o2R.words{o,2})); % phon
    end
end
...
% Get max for each
o2R.phon.max_ph = max(o2R.phon.fon_lev_matrix,[],'all');
o2R.ortho.max_or = max(o2R.ortho.br_lev_matrix,[],'all');

% Divide each by its max, move them into a 0 to 1 scale
o2R.phon.lev_scaled = (o2R.phon.lev_fon) ./ (o2R.phon.max_ph);
o2R.ortho.lev_scaled = (o2R.ortho.ev_br) ./ (o2R.ortho.max_or);
...
o2R.comp_htmp = heatmap(lab_cmp,lab_cmp,comp_scaled,'CellLabelColor','none','Colormap',parula,'Units','pixels','Position',[60 60 650 650]);
...
attempt = struct;
o2.words = sortrows(o2.words,'Item','ascend');

matrix_length = size(o2.words,1);
for n = 1:matrix_length
    for o = 1:matrix_length
        attempt.ortho.lev_all(n,o) = braille_PWLD(char(o2.words{n,1}),char(o2.words{o,1})); % ortho
        attempt.phon.lev_all(n,o) = levPW(char(o2.words{n,2}),char(o2.words{o,2})); % phon
    end
end

max_attempt = max(attempt.phon.lev_all,[],'all');
attempt.phon.lev_scaled = (attempt.phon.lev_all) ./ (max_attempt);

f = figure;
l = string(o2.words{:,1});
attempt.phon.all_htmp = heatmap(l,l,attempt.phon.lev_scaled,'CellLabelColor','none','Colormap',parula,'Units','pixels','Position',[60 60 650 650]);
...
for p = 1:8
    for b = 1:8
        o1.simple_lev(p,b) = lev(char(o1.summary{p,4}),char(o1.summary{b,4}));
    end
end
f = figure;
l = string(o1.summary{:,1});
o1.simple_htmp = heatmap(l,l,o1.simple_lev,'CellLabelColor','none','Colormap',parula,'Units','pixels','Position',[60 60 650 650]);
