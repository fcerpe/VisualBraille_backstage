...
try
    ...
    for i = 1:length(nonwords)
        ...
        targetImg_french = Screen('GetImage', this.win, this.rect*2); 
        WaitSecs(0.3);
     
    end

%     for i = 1:length(nonwords)
%         thisB = char(brailify('éééééé',stimuli));
%         % Blank screen
%         Screen('FillRect', this.win, this.bg_color);
%         
...
%         % Screenshot: first all the screen, then cut what we need 
%         % (PTB was not cooperating, that's why the double step)
%         targetImg_braille = Screen('GetImage', this.win, this.rect*2); 
%         WaitSecs(0.3);
%      
%     end
...
imwrite(targetImg_french,'target.png');

%%

tar_fr = imread('/Users/cerpelloni/Downloads/targetImg_french.png');
tar_br = imread('/Users/cerpelloni/Downloads/targetImg_braille.png');
